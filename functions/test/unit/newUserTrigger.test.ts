...
describe('onUserCreate trigger', () => {
  ...
  it('deve usar \'unknown\' como nome padrão quando displayName for nulo', async () => {
    const userSemNome = {
      uid: 'user456',
  ...
  it('deve logar erro quando addDocument falhar', async () => {
    (addDocument as jest.MockedFunction<typeof addDocument>).mockRejectedValueOnce(new Error('Erro Firestore'));
