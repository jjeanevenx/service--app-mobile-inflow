    ...
    .onUpdate(async (change, context) => {
      try {
        ...
        for (const goal of goalsToProcess) {
          ...
          // learningPath retorna null em caso de falha, ou um objeto de trilha em caso de sucesso
          if (!trilhas) {
            logger.warn(`Não foi possível gerar trilha para a meta: ${goal}`);
            continue;
          }
